...
use crate::application::{CaptchaError, CaptchaResult, CaptchaStore, ChallengeRecord};
...
impl RedisCaptchaStore {
    ...
    fn challenge_key(&self, token: &str) -> String {
        format!("{}:captcha:challenge:{token}", self.key_prefix)
    }

    fn redeemed_key(&self, token_key: &str) -> String {
        format!("{}:captcha:redeemed:{token_key}", self.key_prefix)
    }
}
