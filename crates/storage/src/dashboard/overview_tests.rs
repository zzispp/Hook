...
fn summary_response_exposes_aether_dashboard_metrics() {
    let response = summary_response(
        SummaryRow {
            ...
            avg_first_byte_ms: Some(75.0),
            avg_response_headers_ms: Some(40.0),
            avg_first_token_ms: Some(140.0),
            model_count: Some(4),
    ...
    assert_eq!(response.avg_first_token_ms, Some(140.0));
}
...
fn summary_response_calculates_cache_hit_rate_and_profit() {
    let response = summary_response(
        SummaryRow {
            ...
            avg_first_byte_ms: Some(45.0),
            avg_response_headers_ms: Some(20.0),
            avg_first_token_ms: Some(90.0),
            model_count: Some(2),
    ...
    assert_eq!(response.avg_first_byte_ms, Some(45.0));
    assert_eq!(response.avg_response_headers_ms, Some(20.0));
}

#[test]
fn timeseries_response_preserves_first_byte_cache_hit_rate_and_profit() {
    let response = timeseries_response(
        TimeseriesRow {
            ...
            avg_first_byte_ms: Some(80.0),
            avg_response_headers_ms: Some(30.0),
            avg_first_token_ms: Some(180.0),
        },
        true,
    );
    ...
    assert_eq!(response.avg_first_byte_ms, Some(80.0));
    assert_eq!(response.avg_first_token_ms, Some(180.0));
}
...
fn breakdown_response_preserves_average_latency_and_profit() {
    let response = breakdown_response(
        BreakdownRow {
            ...
            avg_first_token_ms: Some(425.0),
            avg_first_byte_ms: Some(175.0),
        },
        true,
    );
    ...
    assert_eq!(response.avg_first_token_ms, Some(425.0));
}
