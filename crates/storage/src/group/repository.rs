...
use super::{
    ...
    record::{BillingGroupModelRecord, BillingGroupProviderRecord, BillingGroupRecord, billing_group_models, billing_group_providers, billing_groups},
};
...
impl GroupStore {
    ...
    async fn group_from_record(&self, record: BillingGroupRecord) -> StorageResult<BillingGroup> {
        ...
        Ok(domain_group(record, model_ids, provider_ids))
    }
    ...
    async fn groups_from_records(&self, records: Vec<BillingGroupRecord>) -> StorageResult<Vec<BillingGroup>> {
        ...
        let provider_bindings = provider_ids_by_group_codes(codes, self.database.connection()).await?;
        Ok(records
            .into_iter()
            .map(|record| {
                ...
                domain_group(record, model_ids, provider_ids)
            })
            .collect())
    }
}
...
fn domain_group(record: BillingGroupRecord, allowed_model_ids: Vec<String>, allowed_provider_ids: Vec<String>) -> BillingGroup {
    BillingGroup {
        allowed_model_ids,
