...
use super::{
    ...
    record::{BillingGroupModelRecord, BillingGroupRecord, billing_group_models, billing_groups},
};
...
impl GroupStore {
    ...
    async fn group_from_record(&self, record: BillingGroupRecord) -> StorageResult<BillingGroup> {
        ...
        Ok(domain_group(record, model_ids))
    }
    ...
    async fn groups_from_records(&self, records: Vec<BillingGroupRecord>) -> StorageResult<Vec<BillingGroup>> {
        ...
        let bindings = model_ids_by_group_codes(codes, self.database.connection()).await?;
        Ok(records
            .into_iter()
            .map(|record| {
                let model_ids = bindings.get(&record.code).cloned().unwrap_or_default();
                domain_group(record, model_ids)
            })
            .collect())
    }
}
...
fn domain_group(record: BillingGroupRecord, allowed_model_ids: Vec<String>) -> BillingGroup {
    BillingGroup {
        allowed_model_ids,
