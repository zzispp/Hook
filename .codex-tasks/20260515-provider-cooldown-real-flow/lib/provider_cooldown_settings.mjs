...
select request_record_level, request_record_retention_days::text, request_record_payload_retention_days::text,
  ...
  max_request_body_size_kb::text, max_response_body_size_kb::text, scheduling_mode, provider_cooldown_policy
from system_settings where id = 'global';`);
  assert(row, 'system_settings global row should exist');
  return snapshotFromRow(row);
}

export function applyProviderCooldownSettings(db, ctx) {
  db.exec(`
...
set request_record_level = 'full',
    ...
    provider_cooldown_policy = ${q(JSON.stringify(cooldownPolicy(ctx)))},
    updated_at = now()
where id = 'global';`);
}
...
set request_record_level = ${q(snapshot.request_record_level)},
    ...
    provider_cooldown_policy = ${q(snapshot.provider_cooldown_policy)},
    updated_at = now()
where id = 'global';`);
}
...
function snapshotFromRow(row) {
  return {
    ...
    provider_cooldown_policy: row[9],
  };
}

function cooldownPolicy(ctx) {
  return {
    window_seconds: ctx.cooldown.windowSeconds,
    rules: [
      {
        status_code: ctx.cooldown.statusCode,
        failure_count: ctx.cooldown.thresholdCount,
        cooldown_seconds: ctx.cooldown.cooldownSeconds,
      },
    ],
  };
}
