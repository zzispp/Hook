...
pub const ADMIN_API_EXCLUDED_CODES: &[&str] = &["models_public_catalog_read", "wallet_balance_read", "wallet_transactions_read"];
...
mod tests {
    ...
    fn admin_defaults_exclude_user_surfaces() {
        ...
        assert!(!admin_api_codes.contains(&"wallet_balance_read"));
        assert!(!admin_api_codes.contains(&"wallet_transactions_read"));
