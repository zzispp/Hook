...
const ADDITIVE_VERSION: &str = "m20260629_000001_dashboard_recent_error_timing";
...
pub async fn apply(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    for sql in recent_error_timing_sql() {
        execute_sql(manager, sql).await?;
    }
    mark_additive_applied(manager).await
}

fn recent_error_timing_sql() -> [&'static str; 2] {
    [
        "ALTER TABLE IF EXISTS dashboard_recent_error_snapshots ADD COLUMN IF NOT EXISTS response_headers_ms BIGINT NULL",
        "ALTER TABLE IF EXISTS dashboard_recent_error_snapshots ADD COLUMN IF NOT EXISTS first_output_ms BIGINT NULL",
    ]
}
...
#[cfg(test)]
mod tests {
    use super::recent_error_timing_sql;

    #[test]
    fn recent_error_timing_columns_are_additive() {
        let sql = recent_error_timing_sql().join("\n");
        assert!(sql.contains("dashboard_recent_error_snapshots ADD COLUMN IF NOT EXISTS response_headers_ms"));
        assert!(sql.contains("dashboard_recent_error_snapshots ADD COLUMN IF NOT EXISTS first_output_ms"));
    }
}
