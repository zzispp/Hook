...
const DESTRUCTIVE_VERSION: &str = "m20260617_000001_provider_key_capabilities_destructive";
...
pub async fn apply(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    drop_capabilities_column(manager).await?;
    mark_destructive_applied(manager).await
}

pub async fn drop_capabilities_column(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    execute_sql(manager, "ALTER TABLE provider_api_keys DROP COLUMN IF EXISTS capabilities").await
}
