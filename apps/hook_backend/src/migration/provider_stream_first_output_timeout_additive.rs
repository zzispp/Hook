...
const ADDITIVE_VERSION: &str = "m20260628_000001_provider_stream_first_output_timeout";
...
pub async fn apply(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    execute_sql(
        manager,
        "ALTER TABLE IF EXISTS providers ADD COLUMN IF NOT EXISTS stream_first_output_timeout_seconds DOUBLE PRECISION NULL",
    )
    .await?;
