...
fn profile_key_sql() -> [&'static str; 4] {
    [
        ...
        "ALTER TABLE IF EXISTS routing_route_states ADD CONSTRAINT routing_route_states_pkey PRIMARY KEY \
         (profile_id, provider_id, key_id, endpoint_id, global_model_id, client_api_format, provider_api_format, is_stream, route_config_fingerprint, price_config_fingerprint)",
        ...
        "ALTER TABLE IF EXISTS routing_context_route_states ADD CONSTRAINT routing_context_route_states_pkey PRIMARY KEY \
         (profile_id, context_key, provider_id, key_id, endpoint_id, global_model_id, client_api_format, provider_api_format, is_stream, route_config_fingerprint, price_config_fingerprint)",
    ]
}
...
fn index_sql() -> [&'static str; 4] {
    [
        ...
        "CREATE INDEX IF NOT EXISTS index_routing_route_states_by_updated ON routing_route_states (profile_id, last_updated_at DESC)",
        "DROP INDEX IF EXISTS index_routing_context_route_states_by_updated",
        "CREATE INDEX IF NOT EXISTS index_routing_context_route_states_by_updated ON routing_context_route_states (profile_id, context_key, last_updated_at DESC)",
    ]
}
...
mod tests {
    ...
    fn updated_indexes_lead_with_profile_id() {
        ...
        assert!(sql.contains("routing_route_states (profile_id, last_updated_at DESC)"));
        assert!(sql.contains("routing_context_route_states (profile_id, context_key, last_updated_at DESC)"));
    }
}
