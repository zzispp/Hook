...
const ADDITIVE_VERSION: &str = "m20260630_000004_provider_quick_import_group_id";
...
const COLUMN_SQL: &[&str] = &["ALTER TABLE provider_quick_import_keys ADD COLUMN IF NOT EXISTS upstream_group_id VARCHAR(100)"];
...
mod tests {
    ...
    #[test]
    fn group_id_column_is_added_for_existing_quick_import_keys() {
        let sql = COLUMN_SQL.join(" ");

        assert!(sql.contains("ADD COLUMN IF NOT EXISTS upstream_group_id VARCHAR(100)"));
    }
}
