...
function HeroCodeWindow({ siteName }: { readonly siteName: string }) {
  const { t } = useTranslate('landing');
  const [activeSnippet, setActiveSnippet] = useState(0);
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement | null>(null);
  const apiBaseUrl = useApiBaseUrl();
  const snippets = getHeroCodeSnippets(t, siteName);
...
function HeroCopy({ palette, siteInfo }: { readonly palette: HeroPalette; readonly siteInfo: PublicSiteInfo }) {
  const { t } = useTranslate('landing');
  ...
  return (
    <div className="ln-hero-left">
      ...
      <h1 className="ln-hero-headline">
        <span className="ln-hero-headline-line">{siteInfo.site_name}</span>
        <br />
        <span className="ln-hero-headline-line">{t('hero.headlineSuffix')}</span>
      </h1>

      <p className="ln-hero-description">{siteInfo.site_subtitle}</p>
...
export default function Hero({ siteInfo, visualReady }: HeroProps) {
  ...
  return (
    <section className="ln-hero" style={heroStyle}>
      ...
      <div className="ln-hero-content">
        <HeroCopy palette={palette} siteInfo={siteInfo} />
        <div className="ln-hero-right">
          <HeroCodeWindow siteName={siteInfo.site_name} />
        </div>
      </div>
    </section>
  );
