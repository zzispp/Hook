...
const DotField = memo(function DotField({
  ...
  useEffect(() => {
    ...
    if (!context) throw new Error('DotField canvas context is unavailable');
    ...
    function doResize() {
      if (!canvas.parentElement) throw new Error('DotField canvas parent is unavailable');
    ...
    function onMouseMove(e: MouseEvent) {
      if (e.target instanceof Element && e.target.closest('.ln-navbar')) return;
    ...
    function tick() {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (p.sparkle) {
          const hash = Math.abs(Math.sin(i * 2654435761 + Math.floor(frameCount / 8)) * 10000);
          if ((hash % 100) < 3) {
      ...
      >
        <defs>
          <radialGradient id="dot-field-glow">
            <stop offset="0%" stopColor="var(--bg-body)" />
            <stop offset="100%" stopColor="var(--bg-body)" stopOpacity="0" />
          </radialGradient>
        </defs>
        <circle
          ref={glowRef}
