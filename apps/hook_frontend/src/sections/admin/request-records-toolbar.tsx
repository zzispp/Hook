...
export type RequestRecordFilterState = {
  search: string;
  status: string;
  modelId: string;
  providerId: string;
...
export const DEFAULT_REQUEST_RECORD_FILTERS: RequestRecordFilterState = {
  search: '',
...
export function RequestRecordsToolbar({
  filters,
  models,
  providers,
  autoRefresh,
      ...
      <Box
        sx={{
          ...
          gridTemplateColumns: {
            ...
            xl: 'minmax(240px, 1.4fr) repeat(5, minmax(150px, 1fr)) auto',
          },
        }}
      >
        ...
        <SearchSelect
          ...
          label={options.labels.provider}
          options={options.providers}
          onChange={(providerId) => updateFilters({ providerId })}
        />
        <SelectFilter
...
export function toRequestRecordQueryFilters(
  filters: RequestRecordFilterState
): RequestRecordFilters {
  return {
    search: normalizedSearch(filters.search),
    status: optionalFilter(filters.status),
    model_id: optionalFilter(filters.modelId),
    provider_id: optionalFilter(filters.providerId),
...
function useRequestRecordFilterOptions(models: GlobalModelResponse[], providers: Provider[]) {
  ...
  return useMemo(
    () => ({
      ...
      providers: [
        { value: ALL_FILTER_VALUE, label: t('requestRecords.allProviders') },
...
function useRequestRecordFilterUpdater(
  filters: RequestRecordFilterState,
  onChange: (filters: RequestRecordFilterState) => void
) {
  return useCallback(
    (patch: Partial<RequestRecordFilterState>) => {
