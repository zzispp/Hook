...
├── docker-compose.prebuilt.yml # PostgreSQL、Redis 和 GHCR 镜像部署
├── deploy.sh              # Docker Compose 一键部署脚本
├── install.sh             # GitHub Release 二进制包安装脚本
├── update.sh              # Docker Compose 一键更新脚本
├── packaging/             # Release 包模板
...
### Docker Compose（推荐：预构建 GHCR 镜像）

稳定版 tag 会发布多架构镜像 `ghcr.io/zzispp/hook`。`latest` 指向最新稳定版，`edge` 和 `nightly` 指向最新 `main` 构建。这个部署方式会启动 PostgreSQL、Redis 和预构建 Hook 镜像，执行 `migration up`，然后在 `http://127.0.0.1:5555` 提供前端和 API。
...
如果要固定到某个已发布镜像 tag，可设置 `HOOK_IMAGE_TAG`：
...
预构建镜像 Docker Compose 部署后，可在部署目录直接执行：
...
如果你希望在服务器上从当前源码构建镜像，可以使用 Source Build 部署。它会在 Docker 内编译内嵌前端和后端，启动 PostgreSQL 和 Redis，执行 `migration up`，然后在 `http://127.0.0.1:5555` 提供前端和 API。
...
Source Build 部署可使用下面命令更新：

```bash
./update.sh
```

`update.sh` 会执行 `git pull --ff-only`，拉取不需要本地构建的服务镜像，使用当前源码重新构建 Hook 镜像并重建容器。它不会删除 Docker named volumes。

### GitHub Release 二进制包

GitHub Releases 会发布 Linux 与 macOS 平台包：
...
这些包包含 `hook_backend`、`generate_password_hash`、二进制部署配置模板和许可证，适合 PostgreSQL 与 Redis 已经由外部托管的部署方式。它们不会启动 PostgreSQL 或 Redis。
...
安装脚本会下载当前平台包，按 `SHA256SUMS` 校验后安装到 `/opt/hook/releases/<version>`，更新 `/opt/hook/current`，写入 `/etc/hook/config.example.yaml`，并在配置不存在时创建 `/etc/hook/config.yaml`。编辑 `/etc/hook/config.yaml` 后执行：
