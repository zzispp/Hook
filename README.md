...
├── docker-compose.prebuilt.yml # PostgreSQL, Redis, and GHCR image deployment
├── deploy.sh              # One-command Docker Compose deployment
├── install.sh             # GitHub Release binary package installer
├── update.sh              # One-command Docker Compose update
...
### Docker Compose (Recommended: Prebuilt GHCR Image)

Stable release tags publish the multi-arch image `ghcr.io/zzispp/hook`. `latest` points to the latest stable release, while `edge` and `nightly` point to the latest `main` build. This deployment starts PostgreSQL, Redis, and the prebuilt Hook image, runs `migration up`, then serves the frontend and API from `http://127.0.0.1:5555`.
...
After prebuilt-image Docker Compose deployment, run this command in the deployment directory:

```bash
HOOK_COMPOSE_FILES=docker-compose.prebuilt.yml ./update.sh
```

`update.sh` pulls the newest `ghcr.io/zzispp/hook:${HOOK_IMAGE_TAG:-latest}` image, pulls PostgreSQL and Redis base images, and recreates the containers. It does not delete Docker named volumes.
